...
class Logger {
    ...
    public void Debug(string msg) {
        if ((int)LogLevel.DEBUG <= Settings.Default.LOG_LEVEL) {
    ...
    private void CreateLogfile(FileInfo logFile) {
        if (!Directory.Exists(logFile.DirectoryName)) {
            Directory.CreateDirectory(logFile.DirectoryName ?? "./Logs/");
        }

        System.Diagnostics.Debug.WriteLine("Create log file: " + logFile.FullName);
